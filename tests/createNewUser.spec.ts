...
test('Trial pop-up & balance verifications', async ({ }, testInfo) => {
  ...
  expect.soft(page.locator("div[class^='TrialBanner_bannerContainer']")).toBeVisible({ timeout: 10000 });

  if ((await newPageInNewTab.locator("button[class^='TrialBanner_closeButton']").nth(0).isVisible()) &&
    (await newPageInNewTab.locator("div[class^='TrialBanner_icon']").nth(0).isVisible()) &&
    //				(newPageInNewTab.locator("span:has-text('Your free trial ends in 6 days')").isVisible()) &&
    (await newPageInNewTab.locator("text=Your free trial ends in 6 days").first().isVisible()) &&
    (await newPageInNewTab.locator("p:has-text('Top up now to not miss the next stock pick!')").first().isVisible())) {
